...
//
// Measured on 2026-08-06: an invalid key gets an HTTP 101 from OpenAI and the refusal arrives
...
func TestProbeNeverSucceedsWithoutReadiness(t *testing.T) {
	for _, c := range []struct {
		name   string
		script func(f *fakeOpenAI, conn *websocket.Conn)
	}{
		{"101 then the real auth refusal", func(f *fakeOpenAI, conn *websocket.Conn) {
			f.send(conn, `{"type":"error","error":{"type":"invalid_request_error","code":"invalid_api_key","message":"Incorrect API key provided: sk-proj-****ueba"}}`)
			_ = conn.Close(websocket.StatusNormalClosure, "")
		}},
		{"101 then a NORMAL close and nothing else", func(f *fakeOpenAI, conn *websocket.Conn) {
			_ = conn.Close(websocket.StatusNormalClosure, "")
		}},
		{"101 then silence", func(f *fakeOpenAI, conn *websocket.Conn) { time.Sleep(400 * time.Millisecond) }},
		{"101 then malformed JSON", func(f *fakeOpenAI, conn *websocket.Conn) {
			f.send(conn, `{no es json`)
			_ = conn.Close(websocket.StatusNormalClosure, "")
		}},
		{"101 then an unrecognised event", func(f *fakeOpenAI, conn *websocket.Conn) {
			f.send(conn, `{"type":"algo.desconocido","message_type":"algo_desconocido"}`)
			_ = conn.Close(websocket.StatusNormalClosure, "")
		}},
	} {
		t.Run(c.name, func(t *testing.T) {
			f := newFakeOpenAI(t, c.script)
...
func TestProbeSucceedsOnTheSessionConfirmation(t *testing.T) {
	f := newFakeOpenAI(t, func(f *fakeOpenAI, conn *websocket.Conn) { f.ready(conn) })
...
func TestProbeClassifiesPostUpgradeErrorsByCode(t *testing.T) {
	for _, c := range []struct {
		...
		{"the credential", `{"type":"error","error":{"type":"invalid_request_error","code":"invalid_api_key","message":"Incorrect API key provided: sk-proj-****ueba"}}`, stt.ProbeKeyRejected, "invalid_api_key"},
		{"something else", `{"type":"error","error":{"code":"server_error","message":"algo pasó"}}`, stt.ProbeFailed, "server_error"},
	} {
		t.Run(c.name, func(t *testing.T) {
			f := newFakeOpenAI(t, func(f *fakeOpenAI, conn *websocket.Conn) { f.send(conn, c.raw) })
...
func TestProbeWithNoKeyNeverDials(t *testing.T) {
	dialed := false
	f := newFakeOpenAI(t, func(f *fakeOpenAI, conn *websocket.Conn) { dialed = true })
...
func TestProbeReportsARefusedHandshake(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(http.StatusText(c.status), func(t *testing.T) {
			f := newRejectingOpenAI(t, c.status, "")
			res := TestConnection(context.Background(), "clave-secreta-inconfundible", ProbeOptions{Endpoint: f.url})
...
func TestProbeSendsTheSameAuthAsDictation(t *testing.T) {
	...
	if got := f.requestedSubprotocols(); !strings.Contains(got, "openai-insecure-api-key.la-clave") {
		t.Errorf("subprotocols = %q, want the key in openai-insecure-api-key.*", got)
	}
}
