...
// TestConnection reports whether OpenAI accepts this credential.
func TestConnection(ctx context.Context, key string, opts ProbeOptions) stt.ProbeResult {
	...
	conn, resp, err := websocket.Dial(ctx, opts.endpoint(), &websocket.DialOptions{
		// The credential rides in the SUBPROTOCOLS, which is how this service takes it — and the
		// reason its upgrade succeeds even with a bad key: a rejected subprotocol is not a 401.
	...
	case Ready:
		return stt.ProbeResult{
			OK:      true,
			Kind:    stt.ProbeOK,
			Message: "Conexión correcta: OpenAI abrió la sesión con esa clave",
		}
	case Error:
	...
	default:
		return stt.ProbeResult{
			Kind:    stt.ProbeFailed,
			Message: "OpenAI respondió algo inesperado antes de confirmar la sesión",
		}
	}
}
...
func probeMessageFor(kind stt.ProbeKind) string {
	if kind == stt.ProbeKeyRejected {
		return "OpenAI rechazó la API key — revísala en Ajustes"
	}
	return "OpenAI rechazó la conexión"
}
...
func handshakeResult(resp *http.Response, key string) stt.ProbeResult {
	...
	if resp == nil {
		return stt.ProbeResult{
			Kind:    stt.ProbeFailed,
			Message: "no se pudo contactar con OpenAI — comprueba tu conexión a internet",
			Code:    code,
		}
	}
...
func firstMessageFailure(ctx context.Context, err error) stt.ProbeResult {
	if ctx.Err() != nil {
		return stt.ProbeResult{
			Kind:    stt.ProbeFailed,
			Message: "OpenAI aceptó la conexión pero no confirmó la sesión a tiempo",
		}
	}
	...
	if errors.As(err, &closeErr) {
		...
		return stt.ProbeResult{
			Kind:    stt.ProbeFailed,
			Message: "OpenAI cerró la conexión antes de confirmar la sesión",
			Code:    fmt.Sprintf("close %d", int(closeErr.Code)),
		}
	}
	return stt.ProbeResult{
		Kind:    stt.ProbeFailed,
		Message: "se perdió la conexión con OpenAI antes de confirmar la sesión",
	}
}
