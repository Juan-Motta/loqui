...
func describeHandshake(code string, status int, reason string) string {
	...
	// text back on screen. OpenAI's rejection masks the middle of the key and leaves the last four
	// characters INTACT, so redactSecret — which can only remove the exact secret — does not catch it.
	// The comment on redactSecret claimed this was already handled; it was handled for one status only.
