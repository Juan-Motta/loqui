...
// open, failures carry a machine-readable event name. Decode preserves it, but the runtime provider
// does not yet map it into the shared session vocabulary, which is why its collapsed error path remains
// non-retryable (see serverErrorCode).
//
